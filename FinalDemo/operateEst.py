...
class Operate:
    ...
    def save_image(self):
        f_ = os.path.join('lab_output/', f'img_{self.image_id}.png')
        if self.command['save_image']:
	        ...
	        ## KELVIN ADDED ##########################################################
            fileW = "lab_output/images.txt"

            fruit_dict = {"pose":self.ekf.robot.state.tolist(),"imgfname":f_}
        
            if self.image_id == 0:
                with open(fileW, 'w') as fruit_f:
                    json.dump(fruit_dict, fruit_f)
            else:
                with open(fileW, 'a') as fruit_f:
                    fruit_f.write("\n")
                    json.dump(fruit_dict, fruit_f)
    ...
    def init_ekf(self, datadir, ip):
        # use different values if on simulator or physical
        if ip != "localhost":
